const APP_BUILD_VERSION = "2026-09-18.1";
if (window.pdfjsLib) {
...
let currentAdminOrderDraftId = null;
let lines = [];
...
const secureDataCacheMaxAgeMs = 7 * 24 * 60 * 60 * 1000;
...
const saveOrderDraftButton = document.querySelector("#saveOrderDraft");
const saveAdminOrderDraftButton = document.querySelector("#saveAdminOrderDraft");
...
function saveSecureDataCache(userId, result) {
  ...
  try {
    ...
    // Cache local plein ou indisponible (mode privé) : l'app continue de fonctionner avec les
    // donnees deja chargees en memoire, simplement sans acceleration au prochain demarrage.
    try { localStorage.removeItem(secureDataCacheKey(userId)); } catch (cleanupError) {}
  }
}
...
function saveDashboardStatsCache() {
  ...
  try {
    localStorage.setItem(`${dashboardStatsCacheKey}:${currentUser?.id || "anonymous"}`, JSON.stringify({
      savedAt: Date.now(),
    ...
    try { localStorage.removeItem(`${dashboardStatsCacheKey}:${currentUser?.id || "anonymous"}`); } catch (cleanupError) {}
  }
}

function restoreDashboardStatsCache() {
  try {
    const cached = JSON.parse(localStorage.getItem(`${dashboardStatsCacheKey}:${currentUser?.id || "anonymous"}`) || "null");
    ...
    localStorage.removeItem(`${dashboardStatsCacheKey}:${currentUser?.id || "anonymous"}`);
    return false;
  }
}
...
function createClientStatsReportPdf(client, rows) {
  ...
  function circle(cx, yTop, radius, fill = RED) {
    ...
    setColor(fill);
    commands += `${(cx + radius).toFixed(2)} ${cy.toFixed(2)} m ${(cx + radius).toFixed(2)} ${(cy + k).toFixed(2)} ${(cx + k).toFixed(2)} ${(cy + radius).toFixed(2)} ${cx.toFixed(2)} ${(cy + radius).toFixed(2)} c ${(cx - k).toFixed(2)} ${(cy + radius).toFixed(2)} ${(cx - radius).toFixed(2)} ${(cy + k).toFixed(2)} ${(cx - radius).toFixed(2)} ${cy.toFixed(2)} c ${(cx - radius).toFixed(2)} ${(cy - k).toFixed(2)} ${(cx - k).toFixed(2)} ${(cy - radius).toFixed(2)} ${cx.toFixed(2)} ${(cy - radius).toFixed(2)} c ${(cx + k).toFixed(2)} ${(cy - radius).toFixed(2)} ${(cx + radius).toFixed(2)} ${(cy - k).toFixed(2)} ${(cx + radius).toFixed(2)} ${cy.toFixed(2)} c f\n`;
    setColor(previous);
  }
  ...
  function textAt(x, yTop, size, text, options = {}) {
    ...
    const value = shortPdfText(text, options.max || 130);
    const safe = pdfLiteralString(value);
...
async function restoreSession() {
  ...
  if (bootedFromCache) {
    ...
    verifySessionInBackground(savedUser);
    return;
  }
...
async function verifySessionInBackground(savedUser) {
  try {
    ...
    // un delai depasse ne doit jamais lui faire perdre son travail en cours.
    if (error?.servicePayload) {
      showLogin();
...
function loadPriceOfferIntoForm(id) {
  ...
  selectedOffrePrixClient = {
    ...
    sector: offer.sector || "",
    billingAddress: offer.address || "",
    address: offer.address || "",
  };
  if (offrePrixClientSearch) offrePrixClientSearch.value = selectedOffrePrixClient.name;
  offrePrixClientSuggestions?.classList.remove("is-open");
  if (offrePrixSelectedClient) {
    offrePrixSelectedClient.innerHTML = `
      <button class="selected-client-clear" type="button" data-role="offre-prix-clear-client" aria-label="D&eacute;selectionner le client">&times; D&eacute;selectionner</button>
      <strong>${escapeHtml(selectedOffrePrixClient.name)}</strong>
      <span>${escapeHtml(selectedOffrePrixClient.code)}</span>
      <span>${escapeHtml(selectedOffrePrixClient.billingAddress)}</span>
      <span>${escapeHtml(selectedOffrePrixClient.sector)}</span>
    `;
  }
...
async function sendOffrePrixEmail() {
  const rows = getOffrePrixRows();
  const recipients = parseOffrePrixRecipients(offrePrixEmail?.value);
  const effectiveClient = getOffrePrixEffectiveClient();
  if (!effectiveClient) {
    if (offrePrixStatus) offrePrixStatus.textContent = "Sélectionnez un client ou saisissez un nom de prospect.";
    return;
...
function buildOrderDraftSnapshot(existingId) {
  const isoDate = new Date().toISOString();
  const id = existingId || `BROUILLON-${Date.now().toString().slice(-6)}`;
  return {
    id,
    orderNumber: id,
    status: "draft",
...
function storeOrderDraft(order) {
  const orders = getStoredOrders().filter((item) => item.id !== order.id);
  orders.push(order);
  saveStoredOrders(orders);
  recordActivity("Brouillon enregistré", `${order.orderNumber} - ${order.client?.name || "Client non défini"} - ${order.lines.length} ligne(s)`);
  activeHistoryOrderId = order.id;
  renderOrderHistory();
}
...
function saveOrderAsDraft() {
  ...
  const hasContent = selectedClient || orderNote.value.trim() || lines.some((line) => line.ref || Number(line.qty) !== 1);
  if (!hasContent) {
    alert("Rien à enregistrer : sélectionnez un client, ajoutez une ligne ou une note.");
    return;
  }
  const snapshot = buildOrderDraftSnapshot(currentOrderDraftId);
  currentOrderDraftId = snapshot.id;
  storeOrderDraft(snapshot);
  setSyncStatus("local", "Brouillon enregistré dans l'historique");
}

function loadOrderDraftIntoForm(order) {
  currentOrderDraftId = order.id;
  ...
  lines = Array.isArray(order.lines) && order.lines.length
    ? order.lines.map((line) => ({ id: line.id || crypto.randomUUID(), ref: line.ref || "", qty: line.qty ?? 1 }))
    : [{ id: crypto.randomUUID(), ref: "", qty: 1 }];
  renderLines();
  updateSummary();
  activeHistoryOrderId = order.id;
  renderOrderHistory();
...
function renderOrderHistory() {
  ...
  orders.forEach((order) => {
    ...
    const button = document.createElement("button");
    button.type = "button";
    button.className = `history-item${order.id === activeHistoryOrderId ? " is-active" : ""}${isDraft ? " is-draft" : ""}`;
    button.innerHTML = `
      <span>
        <strong>${isDraft ? '<span class="draft-badge">Brouillon</span>' : ""}${escapeHtml(order.client?.name || "Client non défini")}</strong>
  ...
  if (activeOrder.status === "draft") {
    renderDraftDetail(activeOrder, historyDetail);
  } else {
...
function buildAdminOrderDraftSnapshot(existingId) {
  const isoDate = new Date().toISOString();
  const id = existingId || `BROUILLON-${Date.now().toString().slice(-6)}`;
  return {
    id,
    ...
    recipient: schullerOperationsEmail,
    user: currentUser ? { id: currentUser.id, name: currentUser.name, sector: currentUser.sector } : null,
    client: adminOrderSelectedClient ? { ...adminOrderSelectedClient } : null,
    note: adminOrderNote.value,
    total: getAdminOrderTotal(),
    lines: adminOrderLineItems.map((line) => ({ ...line })),
  };
}

function storeAdminOrderDraft(order) {
  const orders = getStoredOrders().filter((item) => item.id !== order.id);
  orders.push(order);
  saveStoredOrders(orders);
  recordActivity("Brouillon enregistré (admin)", `${order.orderNumber} - ${order.client?.name || "Client non défini"} - ${order.lines.length} ligne(s)`);
  activeAdminHistoryOrderId = order.id;
  renderAdminOrderHistory();
}

function saveAdminOrderAsDraft() {
  if (!currentUser) return;
  const hasContent = adminOrderSelectedClient || adminOrderNote.value.trim() || adminOrderLineItems.some((line) => line.ref || Number(line.qty) !== 1);
  if (!hasContent) {
    alert("Rien à enregistrer : sélectionnez un client, ajoutez une ligne ou une note.");
    return;
  }
  const snapshot = buildAdminOrderDraftSnapshot(currentAdminOrderDraftId);
  currentAdminOrderDraftId = snapshot.id;
  storeAdminOrderDraft(snapshot);
  setSyncStatus("local", "Brouillon enregistré dans l'historique");
}

function loadAdminOrderDraftIntoForm(order) {
  if (order.client && order.client.code) {
    selectAdminOrderClient(order.client);
  } else {
    resetAdminOrder();
  }
  currentAdminOrderDraftId = order.id;
  adminOrderNote.value = order.note || "";
  adminOrderLineItems = Array.isArray(order.lines) && order.lines.length
    ? order.lines.map((line) => ({ id: line.id || crypto.randomUUID(), ref: line.ref || "", qty: line.qty ?? 1 }))
    : [{ id: crypto.randomUUID(), ref: "", qty: 1 }];
  renderAdminOrderLines();
  updateAdminOrderSummary();
...
function renderAdminOrderHistory() {
  ...
  orders.forEach((order) => {
    ...
    const button = document.createElement("button");
    button.type = "button";
    button.className = `history-item${order.id === activeAdminHistoryOrderId ? " is-active" : ""}${isDraft ? " is-draft" : ""}`;
    button.innerHTML = `
      <span>
        <strong>${isDraft ? '<span class="draft-badge">Brouillon</span>' : ""}${escapeHtml(order.client?.name || "Client non défini")}</strong>
        <small>${escapeHtml(order.orderNumber)} - ${escapeHtml(order.lines.length)} ligne${order.lines.length > 1 ? "s" : ""}</small>
    ...
    button.addEventListener("click", () => {
      if (isDraft) {
        loadAdminOrderDraftIntoForm(order);
        return;
      }
      activeAdminHistoryOrderId = order.id;
      renderAdminOrderHistory();
  ...
  if (activeOrder.status === "draft") {
    renderDraftDetail(activeOrder, adminOrderHistoryDetail);
  } else {
...
async function generateAdminOrderFiles() {
  ...
  const draftIdToClear = currentAdminOrderDraftId;
  ...
  if (draftIdToClear) deleteAdminStoredOrder(draftIdToClear);
  currentAdminOrderDraftId = null;
  if (orderSendButton) {
...
async function generateOrderFiles() {
  ...
  const draftIdToClear = currentOrderDraftId;
  ...
  if (isTrainingAccount()) {
    ...
    if (draftIdToClear) deleteStoredOrder(draftIdToClear);
...
saveOrderDraftButton?.addEventListener("click", saveOrderAsDraft);
...
offrePrixHistoryList?.addEventListener("click", (event) => {
  ...
  if (deleteId) { deletePriceOfferRecord(deleteId); return; }
  const loadId = event.target.closest("[data-load-price-offer]")?.dataset.loadPriceOffer;
  if (loadId) loadPriceOfferIntoForm(loadId);
});
offrePrixCancelEdit?.addEventListener("click", () => {
  cancelOffrePrixEdit();
  offrePrixLineItems = [];
  renderOffrePrixLines();
  clearOffrePrixClient();
